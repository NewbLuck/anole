...
pub export fn xcbft_query_by_char_support(arg_character: FcChar32, arg_copy_pattern: ?*const FcPattern, arg_dpi: c_long) struct_xcbft_face_holder {
    ...
    patterns.patterns = @ptrCast([*c]?*FcPattern, @alignCast(64, malloc(@sizeOf(?*FcPattern))));
...
pub export fn xcbft_query_fontsearch_all(arg_queries: ?*FcStrSet) struct_xcbft_patterns_holder {
    ...
    current_allocated = 5;
    font_patterns.patterns = @ptrCast([*c]?*FcPattern, @alignCast(64, malloc(@sizeOf(?*FcPattern) *% @bitCast(c_ulong, @as(c_ulong, current_allocated)))));
    ...
    while ((blk: {
        ...
        if (font_pattern != @ptrCast(?*FcPattern, @intToPtr(?*c_void, @as(c_int, 0)))) {
            if ((@bitCast(c_int, @as(c_uint, font_patterns.length)) + @as(c_int, 1)) > @bitCast(c_int, @as(c_uint, current_allocated))) {
                ...
                font_patterns.patterns = @ptrCast([*c]?*FcPattern, @alignCast(64, realloc(@ptrCast(?*c_void, font_patterns.patterns), @sizeOf(?*FcPattern) *% @bitCast(c_ulong, @as(c_ulong, current_allocated)))));
